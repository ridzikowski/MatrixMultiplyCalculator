    ...
    internal class Matrix
    {
        ...
        public Matrix? multiplyThread(Matrix other, int numthreads = 4) {
            ...
            if (other != null && cols == other.rows)
            {
                ...
                {
                    var id = i;
                    threads[i] = new Thread(() => couting(id, this, other, numthreads));
                    threads[i].Name = $"{i}";
                }
        ...
        static void couting(int id, Matrix my, Matrix other, int numthreads)
        {
            for (int v = id; v < my.rows * other.cols; v += numthreads)
            {

                int row = v / other.cols;
                int col = v % other.cols;
                for (int k = 0; k < my.cols; k++)
                    matrix.values[row, col] += my.values[row, k] * other.values[k, col];
            }   
        }
    }
}
